...
char a[300][300];
int sum[300][300], n, ans[300];
map<unsigned short int, unsigned short int> d[251][251];
...
int main()
{
	...
	For (x, 1, n)
		...
		{
			For (y2, 1, y - 1)
				if (sum[x][y] - sum[x][y2 - 1] == 0)
				{
					z = d[x][y][y - y2 + 1] = d[x - 1][y][y - y2 + 1] + y - y2 + 1;
					if (z >= sqr(y - y2 + 1))
						ans[y - y2 + 1]++;
				}
		}
