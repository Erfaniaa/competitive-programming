...
void linearSort(Human people[], int n, int m)
{
	int cnt[100001], cnt2[100001];
	//حد بالای زیادی از سن گرفتم!
	Human ans[100001];
	for (int i = 0; i <= m; i++)
		cnt[i] = 0;
	for (int i = 0; i < n; i++)
	...
	for (int i = n - 1; i >= 0; i--)
	{
		ans[cnt2[people[i].age] - 1] = people[i];
		cnt2[people[i].age]--;
	}
	//آرایه ی مرتب شده از خانه ی صفرم تا خانه ی n-1 ام ans است.
}
