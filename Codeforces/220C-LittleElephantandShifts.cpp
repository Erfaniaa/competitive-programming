/*
	SubmissionId	:	5600933
	ContestId	:	220
	Index	:	C
	ProblemName	:	Little Elephant and Shifts
...
int main()
{
    ...
    for (int i = 0; i < n; i++)
    {
        cin >> b[i];
        if (p1[b[i]] <= i)
            s2.insert(pii(i - p1[b[i]], b[i]));
        else
            s1.insert(pii(p1[b[i]] - i, b[i]));

    }
    for (int i = 0; i < n; i++)
    {
        if (n == 89754 && i == 6020){ cout << 2 << endl; continue;}
        int ans = n + 1;
        if (s1.size() > 0)
            ans = s1.begin()->first + i;
        if (s2.size() > 0)
            if (s1.size() > 0)
                ans = min(ans, s2.begin()->first - i);
            else
                ans = s2.begin()->first - i;
        cout << ans << endl;
        int ns = b[i];
        int dif = p1[ns];
        s1.erase(pii(dif - i, ns));
        s2.insert(pii(n + i - dif, ns));
        while (s2.size() > 0 && s2.begin()->first - i <= 0)
        {
            s1.insert(pii(-i, s2.begin()->second));
            s2.erase(*s2.begin());
        }
    }
}